...
bool canStartPlant(int i,bool manual){PlantState&s=states[i];PlantConfig&c=plants[i];if(i<0||i>=NUM_PLANTS)return false;if(!bootAllowsWatering()||emergencyStop||s.sensorFault||s.waterResponseFault||c.mode==MODE_DISABLED)return false;if(!manual&&c.mode!=MODE_AUTO)return false;if(!manual&&s.moisture>=c.targetLow)return false;if(s.lastWateredMs&&millis()-s.lastWateredMs<c.minIntervalMs)return false;if(s.hourlyPumpMs>=c.maxHourlyMs||s.dailyPumpMs>=c.maxDailyMs)return false;return true;}
...
void startBurst(int i){PlantState&s=states[i];PlantConfig&c=plants[i];digitalWrite(VALVE_PINS[i],RELAY_ON);delay(VALVE_PREOPEN_MS);digitalWrite(PUMP_PIN,RELAY_ON);s.watering=true;s.burstStartMs=millis();}
void finishSession(int i,StopReason reason){PlantState&s=states[i];digitalWrite(PUMP_PIN,RELAY_OFF);delay(VALVE_POSTPUMP_MS);closeAllValves();s.watering=false;s.soaking=false;s.lastWateredMs=millis();s.lastDeliveredMs=s.sessionPumpMs;s.lastStopReason=reason;if(reason!=STOP_WATER_RESPONSE&&!s.sensorFault){clearFaultReason(i);}if(s.sessionPumpMs>0){s.hourlyPumpMs+=s.sessionPumpMs;s.dailyPumpMs+=s.sessionPumpMs;saveRuntime(i);}String h="{\"plantIndex\":"+String(i)+",\"plantName\":\""+safetyName(plants[i].name)+"\",\"durationMs\":"+String(s.sessionPumpMs)+",\"reason\":\""+stopReasonText(reason)+"\",\"moistureStart\":"+String(s.moistureAtSessionStart)+",\"moistureEnd\":"+String(s.moisture)+"}";httpPost("/history/watering",h);s.manualRequest=false;s.manualRequestedMs=0;activePlant=-1;}
...
void evaluateWatering(){
  ...
  if(s.watering){unsigned long run=millis()-s.burstStartMs;if(run>=c.maxBurstMs||millis()-s.sessionStartMs>=c.maxSessionMs){s.sessionPumpMs+=run;s.watering=false;digitalWrite(PUMP_PIN,RELAY_OFF);delay(VALVE_POSTPUMP_MS);closeAllValves();s.soaking=true;s.soakUntilMs=millis()+c.soakMs;return;}if(s.manualRequest&&s.sessionPumpMs+run>=s.manualRequestedMs){s.sessionPumpMs+=run;finishSession(i,STOP_MANUAL_COMPLETE);return;}}
  ...
  if(!s.watering&&millis()-s.sessionStartMs>=c.maxSessionMs){finishSession(i,STOP_SESSION_LIMIT);return;}if(!s.watering&&!s.soaking)startBurst(i);
}

void updateConfig(){for(int i=0;i<NUM_PLANTS;i++){String b;if(!httpGet(String("/config/plants/")+i,b)||!b.length()||b=="null")continue;String n=jsonValue(b,"name");if(n.length())plants[i].name=n;plants[i].targetLow=constrain((int)jsonLong(b,"targetLow",plants[i].targetLow),0,95);plants[i].targetHigh=constrain((int)jsonLong(b,"targetHigh",plants[i].targetHigh),plants[i].targetLow+1,100);plants[i].burstMs=constrain((unsigned long)jsonLong(b,"burstMs",plants[i].burstMs),1000UL,plants[i].maxBurstMs);plants[i].soakMs=constrain((unsigned long)jsonLong(b,"soakSec",plants[i].soakMs/1000UL),10UL,1800UL)*1000UL;plants[i].minIntervalMs=constrain((unsigned long)jsonLong(b,"minIntervalMin",plants[i].minIntervalMs/60000UL),1UL,1440UL)*60000UL;plants[i].mode=parseMode(jsonValue(b,"mode"));}}
...
String statusJson(){String j="{\"firmware\":\""+String(FIRMWARE_VERSION)+"\",\"ota\":true,\"otaPort\":80,\"otaUser\":\""+String(OTA_USER)+"\",\"totalPlants\":"+String(NUM_PLANTS)+",\"systemReady\":"+boolJson(systemReady)+",\"wateringAllowed\":"+boolJson(bootAllowsWatering()&&!emergencyStop)+",\"emergencyStop\":"+boolJson(emergencyStop)+",\"activePlantIndex\":"+String(activePlant)+",\"ip\":\""+deviceIp+"\",\"wifiRssi\":"+String(wifiConnected?WiFi.RSSI():0)+",\"uptimeMin\":"+String(millis()/60000UL)+",\"updatedAtMs\":"+String(millis())+",\"resetReason\":\""+resetReasonText()+"\",\"timeSynced\":"+boolJson(timeSynced)+",\"plants\":[";for(int i=0;i<NUM_PLANTS;i++){if(i)j+=",";j+=plantJson(i);}j+="]}";return j;}
...
void connectWifi(){if(WIFI_SSID==String("").c_str()){}WiFi.mode(WIFI_STA);WiFi.begin(WIFI_SSID,WIFI_PASSWORD);unsigned long start=millis();while(WiFi.status()!=WL_CONNECTED&&millis()-start<15000)delay(250);wifiConnected=WiFi.status()==WL_CONNECTED;deviceIp=wifiConnected?WiFi.localIP().toString():"offline";if(wifiConnected){configTime(19800,0,"pool.ntp.org","time.nist.gov");time_t now=time(nullptr);timeSynced=now>1700000000;}}
