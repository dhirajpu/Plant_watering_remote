(() => {
  ...
  $("registerDeviceBtn").onclick=async()=>{msg("registerStatus","Checking cloud registration…");try{await loadDevices();const id=$("regDeviceId").value.trim().toUpperCase();const d=devices.find(x=>x.deviceId===id);if(!id||!/^ESPBOARD-[A-F0-9]{6}$/.test(id)){msg("registerStatus","Enter a valid Device ID such as ESPBOARD-A1B2C3.",true);return}if(d){msg("registerStatus","Device is registered automatically by the ESP32. You can now generate its enrollment QR.");$("qrDevice").value=id}else{msg("registerStatus","Device not found yet. Power on the ESP32 and connect it to Wi-Fi; it will register automatically.",true)}}catch(e){msg("registerStatus",e.message,true)}};
