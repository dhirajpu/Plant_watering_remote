const CFG=window.PLANT_V3_CONFIG;let controlSessionToken=sessionStorage.getItem('plantV3ControlToken')||'';let controlSessionExpires=Number(sessionStorage.getItem('plantV3ControlExpires')||0);let securityBusy=false;
...
function lockControls(){controlSessionToken='';controlSessionExpires=0;sessionStorage.removeItem('plantV3ControlToken');sessionStorage.removeItem('plantV3ControlExpires');document.body.classList.remove('controls-unlocked');toast('Protected controls locked.')}
...
async function requireControlAuth(reason='Protected action'){
  ...
  try{
    ...
    sessionStorage.setItem('plantV3ControlToken',controlSessionToken);
    sessionStorage.setItem('plantV3ControlExpires',String(controlSessionExpires));
...
function renderOffline(){document.getElementById('systemBadge').textContent='OFFLINE';document.getElementById('systemBadge').className='badge offline';document.getElementById('systemStatus').textContent='Offline';document.getElementById('heartbeat').textContent='Waiting for V3 firmware heartbeat';renderOta({ip:'offline',ota:false})}
...
async function refresh(){if(settingsEditing)return;try{const s=await get('/status');if(!s)throw new Error('No V3 status yet');renderStatus(s)}catch(e){if(Date.now()-lastLiveAt>(CFG.staleTimeoutSec*1000))renderOffline()}}
