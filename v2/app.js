...
window.setMode=async(i,mode)=>{try{const id=await sendCommand('set_mode',{plantIndex:i,mode},mode);if(!id)return;await put(`/config/plants/${i}/mode`,mode);await put('/config/version',String(Date.now()));toast(`Mode changed to ${mode}`);finishCommand(`Mode changed to ${mode}`)}catch(e){finishCommand(`Failed: ${e.message}`)}};
