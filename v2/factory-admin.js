...
function renderNvs(){
  ...
  $('nvsGrid').innerHTML=plants.map((p,i)=>`
    <article class="nvs-config-card">
      <div class="nvs-config-head">
        <div><strong>Plant ${i+1}</strong><span>${esc(p.name||'Unnamed')}</span></div>
        <span class="nvs-key">plantcfg / p${i}</span>
      </div>
      <div class="form-grid">
        <div class="field"><label>Plant name</label><input id="n-name-${i}" value="${esc(p.name||'')}"></div>
        <div class="field"><label>Low %</label><input id="n-low-${i}" type="number" min="0" max="95" value="${Number(p.targetLow||0)}"></div>
        <div class="field"><label>High %</label><input id="n-high-${i}" type="number" min="1" max="100" value="${Number(p.targetHigh||1)}"></div>
        <div class="field"><label>Burst seconds</label><input id="n-burst-${i}" type="number" min="1" max="15" value="${Math.round(Number(p.burstMs||3000)/1000)}"></div>
        <div class="field"><label>Soak seconds</label><input id="n-soak-${i}" type="number" min="10" max="1800" value="${Number(p.soakSec||120)}"></div>
        <div class="field"><label>Min interval minutes</label><input id="n-int-${i}" type="number" min="1" max="1440" value="${Number(p.minIntervalMin||60)}"></div>
        <div class="field"><label>Mode</label><select id="n-mode-${i}">
          <option ${p.mode==='AUTO'?'selected':''}>AUTO</option>
          <option ${p.mode==='MANUAL'?'selected':''}>MANUAL</option>
          <option ${p.mode==='DISABLED'?'selected':''}>DISABLED</option>
        </select></div>
        <div class="field"><label>Dry raw</label><input id="n-air-${i}" type="number" min="80" max="4095" value="${Number(p.airRaw||0)}"></div>
        <div class="field"><label>Wet raw</label><input id="n-wet-${i}" type="number" min="80" max="4095" value="${Number(p.wetRaw||0)}"></div>
      </div>
      <div class="actions">
        <button type="button" onclick="saveNvs(${i})">Save to NVS</button>
        <button type="button" class="secondary" onclick="calibrate(${i},'dry')">Record Dry</button>
        <button type="button" class="secondary" onclick="calibrate(${i},'wet')">Record Wet</button>
      </div>
    </article>`).join('');
}
async function saveNvs(i){const cfg={name:$('n-name-'+i).value.trim(),targetLow:Number($('n-low-'+i).value),targetHigh:Number($('n-high-'+i).value),burstMs:Number($('n-burst-'+i).value)*1000,soakSec:Number($('n-soak-'+i).value),minIntervalMin:Number($('n-int-'+i).value),mode:$('n-mode-'+i).value,airRaw:Number($('n-air-'+i).value),wetRaw:Number($('n-wet-'+i).value)};if(!cfg.name)return toast('Plant name is required.');if(cfg.targetHigh<=cfg.targetLow)return toast('High threshold must be greater than low threshold.');if(cfg.airRaw<80||cfg.airRaw>4095||cfg.wetRaw<80||cfg.wetRaw>4095)return toast('Raw sensor values must be between 80 and 4095.');if(cfg.airRaw===cfg.wetRaw)return toast('Dry raw and wet raw must be different.');try{await command('set_config',{plantIndex:i,...cfg},'Save Plant '+(i+1)+' NVS')}catch(e){}}
async function calibrate(i,type){try{await command(type==='dry'?'calibrate_dry':'calibrate_wet',{plantIndex:i},'Record '+type+' calibration')}catch(e){}}
async function resetFactory(){if(!confirm('Factory reset all plant settings and calibration values?\\n\\nWatering and telemetry history will remain intact.'))return;try{await command('factory_reset_config',{},'Factory reset settings')}catch(e){}}
async function changePassword(){if(!await authenticate())return;const a=prompt('Enter new manufacturer password (8+ characters):');if(a===null)return;if(a.length<8)return toast('Use at least 8 characters.');const b=prompt('Confirm new password:');if(a!==b)return toast('Passwords do not match.');const bytes=new Uint8Array(16);crypto.getRandomValues(bytes);const ns=Array.from(bytes).map(x=>x.toString(16).padStart(2,'0')).join('');const nh=await sha(a+ns);try{await command('change_password',{newSalt:ns,newHash:nh},'Change manufacturer password')}catch(e){}}
function lock(){token='';expires=0;$('adminContent').hidden=true;$('loginSection').hidden=false;$('factoryPassword').value='';toast('Factory portal locked.')}
$('loginBtn').onclick=authenticate;$('factoryPassword').addEventListener('keydown',e=>{if(e.key==='Enter')authenticate()});$('lockBtn').onclick=lock;$('resetBtn').onclick=resetFactory;$('passwordBtn').onclick=changePassword;window.saveNvs=saveNvs;window.calibrate=calibrate;